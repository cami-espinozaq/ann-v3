...
const DialogCarousel = ({ isOpen, setIsOpen, images }: { isOpen: boolean | number; setIsOpen: (open: boolean | number) => void; images: string[] }) => {
  ...
  return (
    ...
    >
      <DialogContent
        className={cn("!max-w-[90vw] md:!max-w-[70vw]", "p-0 md:p-8 flex flex-col items-center border-0 shadow-none", "focus-visible:outline-none")}
      ...
      >
        ...
        <Carousel className="w-[90%] max-h-full mt-4" opts={{ align: "start", duration: 10, startIndex: start }}>
          <CarouselContent className="h-full">
            {images.map((img, i) => (
              <CarouselItem key={i} className="w-full flex items-center max-h-full">
